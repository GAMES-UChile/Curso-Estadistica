...
Este test es usado para verificar si datos multinomiales siguen una distribución dada. Es decir, denotando $p_0 = (p_{01},\ldots,p_{0k})\in[0,1]^k$ y $n\in\N$, estamos interesados en verificar si la distribución $p$ de una variable multinomial 
	\begin{equation}
		H_0:p=p_0\quad \text{v.s.}\quad H_1:p\neq p_0.
	\end{equation}
Para esto consideramos el siguiente estadístico
\begin{equation}
	\label{eq:chi2_stat}
	T = \sum_{i=1}^k \frac{(X_i - n p_{0j})}{n p_{0j}} = \sum_{i=1}^k \frac{(X_i - \E{X_j})}{\E{X_j}}
\end{equation}
donde las esperanzas $\E{X_j} = np_{0j}$ son tomadas con respecto a $\theta_0$. Este estadístico da una medida de discrepancia entre frecuencias observadas y esperadas.

Bajo $H_0$, la distribución asintótica (es decir, cuando el número de observaciones tiene a infinito) es chi cuadrado con $k-1$ grados de libertad $\chi^2_{k-1}$. Consecuentemente, el test rechaza la hipótesis nula si $T\geq \chi^2_{k-1,\alpha}$, donde $\Prob{T\geq \chi^2_{k-1,\alpha}} = \alpha$.
...
\begin{example}[Perritos vagos]
	...
	En base a un estudio preliminares realizado por la fundación para los perros abandonados, se sabe que estas clases de perritos deben distribuir, respectivamente, de acuerdo a \\
	\centerline{\emph{\{9/16, 3/16, 3/16, 1/16\}}.}
	El último censo en este criadero, el cual solo pudo recolectar información de 556 perritos, arrojó los siguiente valores observados: $X=(315, 101, 108,32)$. Esto permite calcular el estadístico del test $\chi^2$ mediante la ecuación \eqref{eq:chi2_stat} obteniendo
	\begin{equation}
		T_{\chi^2} = 0.47.
	\end{equation}
	Para $\alpha=0.05$, tenemos $\chi^2_{3,\alpha} = 7.815>0.47$, por lo que no existe evidencia suficiente para rechazar $H_0$. Es decir, el censo no contradice el estudio preliminar de frecuencias esperadas. 
\end{example}
...
Supongamos que tenemos $X_1,\ldots,X_n\sim F$ iid, donde $F$ es una distribución desconocida sobre $\R$ (tanto en forma como en parámetros). Planteamos además el siguiente problema de test de hipótesis
	\begin{equation}
...
\begin{equation}
 ...
 \end{equation}
 las cuales tienen ley multinomial con parámetro $\theta_i = F(a_i)- F(a_{i-1}),\quad i=1,\ldots,k$. Finalmente, bajo $H_0$ resulta $\theta_i = F_0(a_i)- F_0(a_{i-1}), i=1,\ldots,k$ con lo que podemos aplicar el test $\chi^2$.
